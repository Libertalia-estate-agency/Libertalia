...
const columns = [

  {
    ...
    header: "Update",
    cell: ({ row }) => (
      <div className=" flex gap-4 truncate max-w-sm">
        <span className="text-default-600">{row.getValue("id")}</span>
        <span>{row.getValue("update")}</span>
      </div>
    ),
  },
  {
    ...
    header: "Change",
    cell: ({ row }) => (
      <div className="flex gap-4 max-w-[250px]">
        {row.getValue("change")}
      </div>
    ),
  },
  ...
  },
  {
    ...
    header: "Action",
    cell: ({ row }) => (
      <Link href="#" className="text-primary hover:underline">Delete</Link>
    )
  }
];
